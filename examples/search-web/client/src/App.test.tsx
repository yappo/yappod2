...
describe("yappod search UI", () => {
  ...
  it("shows separate empty, validation, and daemon states", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "検索を実行" }));
    expect(await screen.findByText("一致する文書がありません")).toBeVisible();
  });

  it("shows a local-files path as text when the document has no URL", async () => {
    const mockApi = api({ search: vi.fn().mockResolvedValue(searchResponse({
      results: [{
        id: "lf:1",
        document_id: "lf:1",
  ...
  it("registers a document and searches it without exposing credentials", async () => {
    ...
    await user.type(screen.getByLabelText(/参照URL/), "https://example.test/ux");
