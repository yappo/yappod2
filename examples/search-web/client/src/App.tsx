...
function savedSearchMode(): SearchMode {
  try {
    const saved = window.localStorage.getItem("yappod-search-mode");
...
function saveSearchMode(mode: SearchMode): void {
  try {
    window.localStorage.setItem("yappod-search-mode", mode);
  } catch {
  ...
  return (
    <main id="main-content" className="content search-page">
      <header className="page-heading">
        <p className="eyebrow">文書を探す</p>
        <h1>indexを検索</h1>
        <p>文書のtitle、参照先、本文の抜粋を同じ順序で比較できます。</p>
      </header>
  ...
  return (
    <main id="main-content" className="content register-page">
      ...
      <form className="document-form" onSubmit={submit} noValidate>
        ...
        <div className="field-row">
          <label htmlFor="document-url">参照URL</label>
  ...
  return (
    <>
      ...
      <footer className="site-footer">
        <span>yappod search web example</span>
        <span>接続したindexの文書を検索・参照・登録できます。</span>
      </footer>
    </>
  );
}
