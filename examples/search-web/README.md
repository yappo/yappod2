...
以下の表で「デフォルト・範囲」欄に「`18401`、1〜65535」とある場合、デフォルトは18401、指定できる範囲は
1〜65535です。バッククォートは設定値を示すための表記であり、それ自体がデフォルトを示すものではありません。
...
| `core_port` | `18401`、1〜65535 | frontからcoreへ検索や更新を依頼する専用ポートです。HTTPではありません。 |
| `front_host` | `127.0.0.1` | frontの待ち受け先であり、search-webサーバーの接続先です。 |
| `front_port` | `18400`、1〜65535 | frontのHTTPポートです。 |
| `max_inflight` | `4`、1〜1024 | frontとcoreが、それぞれ処理中として受理するリクエストの件数上限です。 |
| `max_inflight_bytes` | `4194304`、1〜1073741824 | frontは処理中のHTTP本文、coreは処理中の検索・更新データについて、それぞれの合計バイト数を制限します。 |
...
| キー | デフォルト・範囲 | 説明 |
|---|---|---|
| `host` | `127.0.0.1` | search-webサーバーと静的画面の待ち受け先です。 |
| `port` | `4173`、1〜65535 | ブラウザーから接続するポートです。 |
| `yappod_timeout_ms` | `5000`、1〜600000 | search-webサーバーが1回のfront応答を待つ時間です。 |
...
| キー | 必須・デフォルト | 説明 |
|---|---|---|
| `base_url` | 必須 | OpenAI互換APIの基準URLです。search-webサーバーが`chat/completions`を加えます。クエリーとフラグメントは指定できません。 |
...
| キー | デフォルト・範囲 | 説明 |
|---|---|---|
| `enabled` | `false` | `true`ならsearch-webの起動スクリプトが模擬サービスも起動します。 |
...
語彙検索はすべての索引で使えます。ベクトル検索と複合検索は、索引がベクトルを持ち、`[embedding]`の`dimensions`と任意の`model_id`が索引に一致する場合だけ選択できます。search-webサーバーは検索文を埋め込み、frontの`POST /v2/search`へ検索ベクトルを送ります。デフォルトでは50件を文書単位で取得します。

### 質問

search-webサーバーはfrontの`POST /v2/retrieve`へ、最大20件の本文断片、1文書あたり2件の本文断片、コンテキスト
...
| `mode` | 任意、デフォルトは`lexical` | `lexical`、`vector`、`hybrid`です。 |
| `limit` | 任意、デフォルトは`50`、1〜100 | 1ページの最大件数です。 |
| `cursor` | 任意、1〜512文字 | 直前の応答に含まれる`next_cursor`です。 |

ベクトルを使う方式では、search-webサーバーが`query`を埋め込みしてから本体の`POST /v2/search`を呼びます。成功時の応答は本体APIの検索応答と同じです。
...
| `url` | 任意、最大2048文字 | 空白だけなら省略します。URLの構文はsearch-webサーバーでは検証しません。 |
...
| `mode` | 任意、デフォルトは`lexical` | 参照資料の取得に使う検索方式です。 |
