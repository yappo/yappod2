...
`yappo_makeindex`が読むtokenizer、chunking、vector構造用TOMLだけは従来どおり別ファイルで、
application TOMLの`[build].index_config`または`hybrid_index_config`から参照します。

主なsectionは次のとおりです。

| section | 利用者 | 用途 |
|---|---|---|
| `[build]` | index生成・launcher | index構造用TOML、binary、生成index directory |
...
設定例は[`config.example.toml`](config.example.toml)です。portやtimeoutなどの一般設定に環境変数overrideは
ありません。一方、外部APIのsecretはTOMLへ直書きせず、`authorization_token_env`にtokenを保持する環境変数名を
...
有効な`[build].index_directory`を持つ設定を指定して、core、front、production BFF/UIをまとめて起動します。
...
indexがなければ`[build].input`と`[build].index_config`から作成し、存在する場合はそのまま起動します。
停止も同じ設定を指定します。
