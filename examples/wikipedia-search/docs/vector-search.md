...
この文書は、`wikipedia_data.py fetch-api`で取得した最大1,000件の記事をvector化し、yappod2の
vector検索とhybrid検索を確認するための手順です。既定例はローカルで動くGoogle EmbeddingGemmaと
LM StudioまたはOllamaです。必要に応じてOpenAIの`text-embedding-3-small`も利用できます。
...
文章からvectorを生成する処理はLM Studio、Ollama、OpenAIのいずれかが担当します。SQLiteのvector拡張と同様に、
...
python3 wikipedia_data.py fetch-api \
  --limit 1000 \
  --output ../data/wikipedia-search/documents.ndjson
```
...
../../build/yappo_makeindex prepare \
  ...
  --input ../data/wikipedia-search/documents.ndjson \
  --output ../data/wikipedia-search/passages.ndjson
```
...
`wikipedia-search.toml`へ次の`[embedding]`を設定します。`model`はLM Studioに表示されたEmbeddingGemmaの
...
python3 wikipedia_data.py embed \
  --documents ../data/wikipedia-search/documents.ndjson \
  --passages ../data/wikipedia-search/passages.ndjson \
  --output ../data/wikipedia-search/documents.vector.ndjson
```
...
既定のURLは`http://127.0.0.1:11434`です。`wikipedia-search.toml`の`[embedding]`をOllama用に設定してから
adapterを実行します。
...
Bearer tokenは`wikipedia-search.toml`へ直接設定します。秘密を含む設定はcommitせず、所有者だけが読める
permissionにしてください。完全URLの`endpoint_url`はpathを追加せずそのまま使用されます。
...
[usage_log]
path = "../data/wikipedia-search/api-usage.jsonl"
```
...
`usage_log.path`は`wikipedia-search.toml`基準で解決します。成功したembedding requestごとにUTC日時、source、
service、operation、provider、model、API応答の`usage`（未返却時は`null`）をJSONLへappendします。Web BFFの
chatとembedding requestも同じ形式で記録します。prompt、token、vector、回答本文、認証情報、endpointは
記録しません。ログ書き込み失敗はstderrへのwarningだけで処理を継続します。

`base_url`と`endpoint_url`はどちらか一方だけを指定します。HTTPSは常に許可します。HTTPは`localhost`、
IPv4 loopback `127.0.0.0/8`、RFC 1918、IPv6 loopback `::1`、RFC 4193 ULAだけ許可し、その他のhostnameと
literal IPにはHTTPSを必須とします。DNS解決によるprivate判定は行いません。認証はOpenAI互換Bearer方式が
対象であり、Azure固有の`api-key`方式は対象外です。

## embedding adapterの検証内容

adapterはproviderの違いを吸収し、次の処理を行います。

1. `../data/wikipedia-search/documents.ndjson`の元文書を`id`で保持する。
2. `../data/wikipedia-search/passages.ndjson`を読み、`document_id`と`ordinal`の順序を保持する。
3. 設定したprofileに従い、記事titleとpassage textをbatch送信する。
4. response件数が送信件数と一致することを確認する。
5. 全vectorが設定次元で、全要素がfinite numberであることを確認する。
6. documentごとにordinal順の二次元配列へまとめ、元文書へ`vectors`として追加する。
7. 全documentに必要なvectorが揃ってから、一時ファイルを`../data/wikipedia-search/documents.vector.ndjson`へatomic renameする。
...
既定例の内側配列は768要素です。document vectorを全passageへ複製してはいけません。外側の件数と
...
input = "../data/wikipedia-search/documents.vector.ndjson"

[index]
directory = "../data/wikipedia-search/index-embeddinggemma"
```
...
同じapplication設定でdaemonとWeb UIを起動します。

```sh
./scripts/start_demo.sh --config ./wikipedia-search.toml
```
...
```sh
./scripts/stop_demo.sh --config ./wikipedia-search.toml
```
...
検索結果は1ページ50件で、「続きを読み込む」も50件単位です。RAGは最大20 passage、context上限32 KiBで
取得します。title、URL、snippetという情報構造は変わりません。詳細を開いた場合だけlexical、vector、
fused scoreの違いを確認できます。RAGでは選択モードを`/v2/retrieve`にも使うため、回答形式は同じでも
参照資料の選ばれ方が変わります。

Web UIで意味検索・hybrid検索・vector RAGを使う場合も、index生成と同じ`wikipedia-search.toml`の
`[embedding]`を使用します。回答生成の`[llm]`とは独立した設定です。
全キーの意味は[共通Web UIの共有設定](../../search-web/README.md#共有設定)を参照してください。
...
OpenAIの場合は前述の`provider = "openai"`、完全`endpoint_url`、`prompt_profile = "plain"`、
`authorization_token_env`を使います。Web UIとPython adapterは同じprovider、model、dimensions、prompt profileを
参照するため、index作成時と検索時の設定を一致させてください。
...
| Web UIで意味検索を選べない | `wikipedia-search.toml`の`[embedding]`、indexのvector設定、次元数の一致を確認する |
