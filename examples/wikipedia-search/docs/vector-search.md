...
```sh
curl -sS http://127.0.0.1:10080/v2/search \
  -H 'Content-Type: application/json' \
...
curl -sS http://127.0.0.1:10080/v2/retrieve \
  -H 'Content-Type: application/json' \
...
export EMBEDDING_MODEL='LM Studioに表示されたEmbeddingGemmaのmodel ID'
export EMBEDDING_INDEX_MODEL_ID=embeddinggemma-300m-768-local-v1
export EMBEDDING_DIMENSIONS=768
export EMBEDDING_PROFILE=embeddinggemma
...
| Web UIで意味検索を選べない | BFFのembedding環境変数、indexのvector設定、次元数の一致を確認する |
