...
OpenAIを使う場合は`profile = "plain"`とし、設定した`dimensions`をAPI requestにも送信します。
...
Bearer tokenを環境変数へ設定し、`web/config.toml`では環境変数名だけを参照します。完全URLの
`endpoint_url`はpathを追加せずそのまま使用されます。
...
`usage_log.path`は`web/config.toml`基準で解決します。成功したembedding requestごとにUTC日時、source、
...
OpenAIの場合は前述の`provider = "openai"`、完全`endpoint_url`、`profile = "plain"`、
`authorization_token_env`を使います。Web UIとPython adapterは同じprovider、model、dimensions、profileを
