...
この文書では、`examples/local-files/local_files.py`が読み取る設定を説明します。索引形式、デーモン、Web、埋め込み、LLMの共通項目は[Yappod2設定リファレンス](../../../docs/configuration.md)を参照してください。
...
| `batch_size` | 既定値`16`、1〜1024です。 |
| `timeout_ms` | 既定値`60000`の正の整数です。1回のHTTPリクエストに適用します。 |
| `prompt_profile` | `plain`または`embeddinggemma`です。既定値は`plain`です。 |
...
`index.directory`も`build`と`all`で必要です。`build --target lexical`と`build --target rag`では`[vector].enabled = false`、`build --target hybrid`では`[vector].enabled = true`にします。`rag`はパッセージ成果物を準備したうえで、ベクトルを持たない検索索引も作ります。
