...
```sh
python3.10 examples/local-files/local_files.py convert \
...
- Python 3.10以上
- PDFやOffice文書を扱う場合は`requirements.txt`のPython依存package
...
Python executable名は環境に合わせて変更してください。Tikaは自動downloadしません。Apache公式から
取得してchecksumを確認し、後述の`extract.tika_command`へ絶対pathを設定します。
