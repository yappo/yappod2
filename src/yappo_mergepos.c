...
/*キーワードのマージ*/
void core(YAPPO_DB_FILES *in_p, YAPPO_DB_FILES *out_p, int start, int end, int byte) {
  int i, ii, ret;
  unsigned long ui;
  YAPPO_DB_FILES *pos;
  ...
  buf = (char **)mmap(0, sizeof(char) * (in_p->total_keywordnum + 1), PROT_WRITE | PROT_READ,
                      MAP_SHARED, buf_fd, 0);
  buf_len = (long *)mmap(0, sizeof(long) * (in_p->total_keywordnum + 1), PROT_WRITE | PROT_READ,
                         MAP_SHARED, buf_len_fd, 0);
