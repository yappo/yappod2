...
|---|---|---|---|---|---|
| `operation` | 文字列 | `upsert` | なし | 必須 | 同じ`id`の古い文書を置き換えます。 |
| `id` | 文字列 | 1〜255バイト | なし | 必須 | 索引内で一意な文書IDです。 |
| `url` | 文字列 | 0〜8192バイト | 空文字列 | 任意 | 検索結果と取得結果へ返す出典URLです。URLとしての構文検証はしません。 |
| `title` | 文字列 | 0〜255バイト | 空文字列 | 任意 | 語彙検索のtitleフィールドと表示に使います。 |
| `body` | 文字列 | 0〜1048576バイト | なし | 必須 | 検索対象本文です。空文字列は形式上受理されますが、本文断片を生成できないため実用上は本文を入れてください。 |
| `metadata` | JSONオブジェクト | 正規化後0〜1048576バイト | `{}` | 任意 | 文書コンポーネントへ保存し、`filterable_fields`で選んだ値を絞り込み索引へ入れます。現行の検索レスポンスには返しません。 |
| `updated_at_unix_ms` | 整数 | 0以上 | `0` | 任意 | 更新時刻をUnix epochからのミリ秒で表します。値の意味は呼び出し側で管理します。 |
| `vectors` | 浮動小数点数の二次元配列 | 行数は生成される本文断片数、各行の要素数は`vector.dimensions`と同数、各要素はfloat32で表現できる有限値 | なし | ベクトル対応索引では必須 | `[chunking]`で生成される本文断片順に並べます。 |
...
| キー | データ型 | 入力可能値 | デフォルト値 | 必須 | 説明 |
|---|---|---|---|---|---|
| `operation` | 文字列 | `delete` | なし | 必須 | `id`で指定した文書を検索結果から除外します。 |
| `id` | 文字列 | 1〜255バイト | なし | 必須 | 削除する文書のIDです。 |
...
|---|---|---|---|---|---|
| `--config CONFIG` | 文字列 | 読み取り可能なアプリケーションTOML | なし | 必須 | `[chunking]`を含む必須セクションをすべて読みます。 |
| `--input INPUT` | 文字列 | 読み取り可能な通常ファイル | なし | 必須 | `--input-format`に応じてNDJSONまたはTSVとして読みます。標準入力を表す`-`は使用できません。 |
| `--output OUTPUT` | 文字列 | 作成または上書き可能な通常ファイル | なし | 必須 | 本文断片NDJSONを書きます。標準出力を表す`-`は使用できません。既存ファイルは上書きするため、残す必要があるファイルを指定しないでください。 |
| `--input-format FORMAT` | 文字列 | `ndjson`、`tsv` | `ndjson` | 任意 | 入力ファイルの形式を指定します。 |
...
| オプション | データ型 | 入力可能値 | デフォルト値 | 必須 | 説明 |
|---|---|---|---|---|---|
| `--config CONFIG` | 文字列 | 読み取り可能なアプリケーションTOML | なし | 必須 | `[index].directory`から新しい索引の作成先を読みます。 |
| `--input INPUT` | 文字列 | `upsert`だけを含む、読み取り可能なNDJSONファイル | なし | 必須 | 空行、`delete`、未知のフィールドを拒否します。 |

### 処理と上限

`build`は初回の索引作成に使うため、入力ファイルを最後まで読みます。多数の文書は最大10000件ずつ処理し、必要に応じて
複数のセグメントへ分けて保存します。通常は各コンポーネントの最大ペイロードが128 MiBへ近づくように分割し、
小さい末尾セグメントは結合後が192 MiB以内なら直前へまとめます。文書数そのものに固定の上限はありませんが、
各索引ファイルの256 MiB上限、最大セグメント数、利用可能なディスク容量とメモリー量による制限を受けます。
...
| オプション | データ型 | 入力可能値 | デフォルト値 | 必須 | 説明 |
|---|---|---|---|---|---|
| `--config CONFIG` | 文字列 | 読み取り可能なアプリケーションTOML | なし | 必須 | `[index].directory`から更新対象の索引を読みます。 |
| `--input INPUT` | 文字列 | 1〜10000行の`upsert`または`delete`を含む、読み取り可能なNDJSONファイル | なし | 必須 | 1行を文書1件の登録、更新、削除として数えます。 |

`update`は入力した1〜10000件をまとめて検証し、すべて正常な場合だけ一度に公開します。10001件以上を
一度に渡した場合は、世代を公開する前に入力全体を拒否します。この上限は1回の更新処理に対するものであり、
索引全体の文書数を制限するものではありません。
...
| オプション | データ型 | 入力可能値 | デフォルト値 | 必須 | 説明 |
|---|---|---|---|---|---|
| `--config CONFIG` | 文字列 | 読み取り可能なアプリケーションTOML | なし | 必須 | `[index].directory`から検証対象の索引を読みます。 |
...
| `--scope SCOPE` | 文字列 | `documents`、`passages` | `documents` | 任意 | 通常の検索結果一覧には`documents`、RAG用の参照箇所を探す場合には`passages`を使用します。 |
| `--limit NUMBER` | 整数 | 1〜100 | `10` | 任意 | 返す検索結果の最大件数です。 |
...
1. `compaction.lock`を取得し、compact同士を直列化します。
2. 短時間`writer.lock`を取得し、マニフェストから最大8個、コンポーネントの記録サイズ合計512 MiB以内を目安に、隣接するセグメント範囲を選びます。単独セグメントが512 MiBを超える場合は、その1個を選べます。
3. `writer.lock`を解放し、選択範囲内で最後に現れる文書または削除標識を、新しい一つ以上の`compact-*`セグメントへ再構築します。
4. `writer.lock`を再取得し、選択したdescriptorが現在のマニフェストでも同じ位置に連続していることを確認します。
5. 選択範囲だけをcompactセグメントへ置き換え、範囲外の既存セグメントと、構築中に追加された新しいセグメントを同じ順序で残して次のマニフェストを公開します。
6. 置き換えた旧セグメントディレクトリを回収し、ロックを解放します。

新しいセグメントの構築中は通常の更新を妨げません。構築中に更新が末尾へ追加されても、選択範囲が変わっていなければ、その更新を残したまま公開します。選択範囲が変わった場合は、作成したセグメントを公開せずに失敗します。

選択範囲が現在のマニフェスト全体でない場合、範囲より前にある文書を隠すために必要な削除標識をcompactセグメントへ引き継ぎます。全体を置き換える場合だけ、削除済み文書と削除標識を両方とも除去します。複数回実行すると、別の隣接範囲を段階的に整理できます。

コンパクション中は`<index-directory>/compaction.state`へ状態を書きます。公開前に失敗した場合は開始時のマニフェストを維持します。途中でプロセスが終了した場合、ヘルスチェックとメトリクスは実行中PIDの不在から`interrupted`を報告します。
...
`documents`と`passages`は今回再構築した選択範囲の件数です。`removed_segments`はコンパクション開始前の不要データ回収と、今回置き換えた旧セグメントの回収で削除したセグメントの合計です。
...
coreは`daemon.worker_threads`本のワーカースレッドで内部接続を受け、1秒ごとに新しいマニフェストを確認します。既定値は16本です。公開済み世代を検出すると、検証に成功したスナップショットへ切り替えます。`SIGTERM`または`SIGINT`で待ち受けを閉じて終了します。
