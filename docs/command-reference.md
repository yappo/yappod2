...
```json
{"operation":"upsert","id":"doc-1","url":"https://example.test/1","title":"検索入門","body":"検索対象となる本文です。","metadata":{"language":"ja"},"updated_at_unix_ms":1784516400000}
```
...
| `url` | 任意 | 文字列、最大8192バイト、既定値`""` | 検索結果と取得結果へ返す出典URLです。URLとしての構文検証はしません。 |
| `title` | 任意 | 文字列、最大255バイト、既定値`""` | 語彙検索のtitleフィールドと表示に使います。 |
| `body` | 必須 | 文字列、最大1048576バイト | 検索対象本文です。空文字は形式上受理されますが、パッセージを生成できないため実用上は本文を入れてください。 |
| `metadata` | 任意 | JSONオブジェクト、正規化後最大1048576バイト、既定値`{}` | 文書コンポーネントへ保存し、`filterable_fields`で選んだ値を絞り込み索引へ入れます。現行の検索レスポンスには返しません。 |
| `updated_at_unix_ms` | 任意 | 0以上のJSON整数、既定値`0` | 更新時刻をUnix epochからのミリ秒で表します。値の意味は呼び出し側で管理します。負の値は拒否されます。 |
| `vectors` | ベクトル対応索引では必須 | 数値の二次元配列 | `[chunking]`で生成されるパッセージ順に並べます。行数はパッセージ数、各行の要素数は`vector.dimensions`と一致させ、全要素を有限な浮動小数点数にします。 |
...
文書入力を`[chunking]`で分割し、埋め込み処理などへ渡すパッセージNDJSONを作ります。この出力はパッセージ用の中間形式であり、`yappo_makeindex build`へそのまま渡す文書NDJSONではありません。
...
| `--output OUTPUT` | 必須 | パッセージNDJSONの出力ファイルです。標準出力の`-`は実装されていません。現行実装は`w`で開くため、既存ファイルを上書きします。必要なファイルを指定しないでください。 |
| `--input-format` | 任意 | `ndjson`または`tsv`です。既定値は`ndjson`です。 |

### NDJSON入力と出力

NDJSON入力は前述の`upsert`または`delete`です。`upsert`一件からパッセージごとに次の行を出力します。
...
| `passage_id` | 文書ID、`ordinal`、パッセージ本文からFNV-1a方式で計算した64 bit値を、`p-`に16桁の小文字16進数を続けた形で表します。上の値は、この例の文書IDと本文から実装と同じ手順で計算した値です。 |
| `ordinal` | 文書内の0始まりの順番です。 |
...
`--index`は受理しません。出力先は必ずTOMLの`[index].directory`で指定します。

### 処理と上限

入力全体の操作件数を100件に制限してはいません。実装は最大10000件ずつ内部バッチに分け、入力が終わるまで読み続けます。各内部バッチは必要に応じて複数セグメントへ分割されます。文書数、パッセージ数、個々のコンポーネントの256 MiB上限、最大セグメント数、利用可能なディスクとメモリーは別途適用されます。
...
`update`では、入力NDJSONの1行を1件の操作として数え、1回のコマンドで1〜100件を受理します。この上限は既存索引を一度に不可分に更新するバッチの上限です。索引全体の文書数や、`build`入力全体の上限ではありません。100件を超える更新は、重複IDや順序を考慮して呼び出し側で複数回に分けます。
...
| `--input INPUT` | 必須 | 1〜100行の`upsert`または`delete` NDJSONです。 |

`--index`は受理しません。

### 不可分性と競合

入力をすべて解析し、同じバッチ内の文書ID重複、パッセージ、ベクトル、セグメント容量を検証してからマニフェストを公開します。一行でも不正なら、バッチ全体を公開しません。更新中は索引ディレクトリの書き込みロックを取得します。公開直前に世代が変わった場合は競合として失敗し、作成途中のセグメントを回収します。
...
```json
{"generation":3,"accepted":2,"upserts":1,"deletes":1,"segment_ids":["seg-00000000000000000003-AbCd12"]}
```
...
`--index`は受理しません。TOMLの`[index].directory`を検証します。
...
- ANNファイルの次元、距離方式、件数と`vectors.yap2`との整合性
...
| `--scope` | 任意 | `documents`または`passages`です。既定値は`documents`です。 |
| `--limit` | 任意 | 1〜100の整数です。既定値は`10`です。 |

`search` CLIはHTTP検索処理を同じプロセス内で実行します。`yappod_core`や`yappod_front`を起動する必要はありません。metadata filter、フレーズ、operator、cursorを指定するCLIオプションはありません。それらが必要な場合は`POST /v2/search`を使用します。
...
2. 最新のマニフェストで有効な文書とパッセージだけを収集します。
...
```json
{"generation":4,"documents":12000,"passages":36000,"removed_segments":7,"segment_ids":["compact-00000000000000000004-XyZ123"]}
```
...
| `--index INDEX_DIR` | `--config`を使わない場合に必須 | 索引を直接指定します。実行時設定は既定値になります。 |
| `--port PORT` | `--index`形式だけで任意 | 内部フレームのポートです。既定値は`18401`です。1〜65535を指定します。 |
...
coreは外部クライアント向けHTTPサーバーではありません。通常の利用者はfrontへ接続してください。内部プロトコルは[`yappod_core`内部プロトコル](yappod-core-protocol.md)で説明します。
...
| `--port PORT` | 直接指定形式で任意 | HTTPポートです。既定値`18400`、範囲1〜65535です。 |
| `--core-port PORT` | 直接指定形式で任意 | coreの内部ポートです。既定値`18401`、範囲1〜65535です。 |
...
frontは16本のワーカースレッドでHTTP/1.xリクエストを処理し、検索、RAG向け取得、文書更新をcoreへ内部フレームで転送します。`passages:prepare`はfrontのプロセス内で実行します。ヘルスチェックとメトリクスもfrontが返します。対応するHTTP仕様は[`yappod_front` APIリファレンス](yappod-front-api.md)、メトリクスは[監視とメトリクス](observability.md)を参照してください。
