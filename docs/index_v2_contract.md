...
この文書は、v2索引を生成・読み込むコンポーネント間の契約です。Task 2では契約と検証だけを追加し、segmentの生成・検索・既存v1形式との接続は行いません。
...
設定ファイルはindex directory直下の`config.toml`です。TOML parserとfail-fastの実装は後続タスクですが、キーと制約はここで固定します。
...
- checksumはsegment一式を対象とするSHA-256のhex表現を予定します。C契約では32-byte値として保持し、計算方法はsegment writerタスクで実装します。
