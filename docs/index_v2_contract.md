...
- `segments`配列はpublish順（古い順から新しい順）です。重複document IDとtombstoneは末尾側segmentを優先して解決します。
...
- header generationはimmutable componentを作成したgenerationで、1以上かつ参照manifest generation以下です。
...
| 12 | 8 | component creation generation (`uint64`, LE, non-zero) |
| 20 | 8 | payload bytes (`uint64`, LE) |
| 28 | 4 | payload CRC32C (`uint32`, LE) |

YAP2 headerのfile typeはterms=1、postings=2、positions=3、documents=4、metadata=5、vectors=6、tombstones=7です。headerの予約領域はなく、未知のfile typeやversionは読み込み時に拒否します。manifest component type 8は再生成可能な外部形式`vectors.usearch`専用で、YAP2 headerは持たずchecksumとfile sizeを検証します。
...
`YAP_V2_snippet_window`はmarkerを追加せず、元textを参照する最大指定grapheme数のbyte viewを
返します。長いdocumentでも本文全体のcopyやgrapheme offset配列を作りません。
