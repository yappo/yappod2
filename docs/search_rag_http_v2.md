...
`yappod_core` へ転送します。coreは起動時に
`config.toml`、`manifest.json`、componentを検証し、一つのgenerationの
snapshotと同じsegment順のlexical、vector/ANN、metadata readerを開いたまま保持して
...
tombstoneを一つのimmutable delta segmentへ書き、新segmentだけを検証します。全component完成後に
generation CASでmanifestをatomic publishするため、失敗時にpartial batchは検索されません。
成功後は既存segment readerを再利用し、新segmentを追加したsnapshotへ切り替えます。
writer競合は`409 generation_conflict`、index I/O/検証不能は`503`です。
